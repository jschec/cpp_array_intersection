/**
 * IntersectionMethods.cpp
 *
 * Implementations for methods used for identifying common elements between two
 * vectors.
 * 
 * Joshua Scheck
 * 2021-06-10
 ...
 */
//template <typename T>
std::unordered_set<char> bruteForceIntersection(const std::vector<char> &vectOne,
    const std::vector<char> &vectTwo) 
{
    ...
    for (unsigned int vectOneIdx = 0; vectOneIdx < vectOne.size();
        vectOneIdx++)
    {
        for (unsigned int vectTwoIdx = 0; vectTwoIdx < vectTwo.size(); 
            vectTwoIdx++)
        {
...
/**
 * Execute binary search iteratively to determine common values between both
 * vectors.
 * 
...
/**
 * Leverages multiple threads to speed up linear both vectors to find shared
 * elements.
 * 
 * @param   vectOne the first vector of elements to be searched
 * 
    ...
    const std::vector<T> &vectTwo)
{
    ...
    for (unsigned int idx = 0; idx < vectOne.size(); idx++) {
        ...
        for (unsigned int threadIdx = 0; threadIdx < threads.size();
            threadIdx++)
        {
